...
**SE/Ultima mode**
BeeGees supports single-end reads generated by Ultima Genomics (UG100) sequencing as a third preprocessing mode, distinct from the two PE ('concat' and 'merge') modes. 
 
**Key differences from PE mode:**
- Only one preprocessing path.
- fastp is run in SE mode with Ultima-specific settings: poly-X tail trimming (targeting poly-T 3' artefacts characteristic of Ultima flow-based chemistry), poly-G trimming disabled (Ultima is not a two-colour system), and deduplication enabled.
